...
OpenSeadragon.Viewer.prototype.setFilterOptions = function(options) {
    if (!this.filterPluginInstance) {
        options = options || {};
        options.viewer = this;
        this.filterPluginInstance = new OpenSeadragon.FilterPlugin(options);
    } else {
 ...
 */
OpenSeadragon.FilterPlugin = function(options) {
    options = options || {};
    if (!options.viewer) {
...
function setOptions(instance, options) {
    ...
    instance.filters = !filters ? [] :
        OpenSeadragon.isArray(filters) ? filters : [filters];
    for (let i = 0; i < instance.filters.length; i++) {
        let filter = instance.filters[i];
        if (!filter.processors) {
            throw new Error('Filter processors must be specified.');
        }
        filter.processors = OpenSeadragon.isArray(filter.processors) ?
    ...
    if (options.loadMode === 'sync') {
        ...
        for (let i = 0; i < instance.filters.length; i++) {
            ...
            if (OpenSeadragon.isArray(filter.items)) {
...
function getFiltersProcessors(instance, item) {
    ...
    for (var i = 0; i < instance.filters.length; i++) {
        ...
        if (!filter.items) {
            ...
            (OpenSeadragon.isArray(filter.items) && filter.items.indexOf(item) >= 0) ) {
...
OpenSeadragon.Filters = {
    ...
    CONVOLUTION: function(kernel) {
        if (!OpenSeadragon.isArray(kernel)) {
