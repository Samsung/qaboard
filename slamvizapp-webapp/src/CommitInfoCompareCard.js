  ...
  return <Card elevation={4}>
    <div style={{display:'flex', justifyContent: 'space-between', alignItems: 'center'}}>
      ...
      <div style={{flex:'1 1 auto'}}>
          ...
          <div style={{marginTop: '10px', textAlign: 'right'}}><DoneAtTag commit={ref_commit} /> {ref_ci_batch.valid_slam_outputs>0 && <Tag intent={Intent.SUCCESS}>{ref_ci_batch.valid_slam_outputs} outputs</Tag>} {ref_ci_batch.failed_slam_outputs>0 && <Tag intent={Intent.DANGER}>{ref_ci_batch.failed_slam_outputs} crashed</Tag>} {ref_ci_batch.running_slam_outputs>0 && <Tag>{ref_ci_batch.running_slam_outputs} running</Tag>} {ref_ci_batch.pending_slam_outputs>0 && <Tag>{ref_ci_batch.pending_slam_outputs-ref_ci_batch.running_slam_outputs} pending</Tag>} <Tag intent={Intent.PRIMARY}>Reference</Tag></div>
