...
const CommitInfoCompareCard = ({new_commit, ref_commit, onConfirmReference}) => {
    ...
    <div style={{display:'flex', justifyContent: 'space-between', alignItems: 'center'}}>
      <div style={{flex:'1 1 auto', minWidth: '450px'}}>
        <h1 style={{display: 'flex', alignItems: 'baseline'}}><Avatar href={`/committer/${new_commit.committer_name}`} alt={new_commit.committer_name} src={new_commit.committer_avatar_url} />{new_commit.type==='git' ? new_commit.id.substring(0,8) : new_commit.id} </h1>
          ...
          <br/>
          <div style={{marginTop: '10px'}}><DoneAtTag commit={new_commit} /> <Tag intent={Intent.SUCCESS}>{new_ci_batch.valid_slam_outputs} outputs @CI</Tag> {new_ci_batch.failed_slam_outputs>0 && <Tag intent={Intent.DANGER}>{new_ci_batch.failed_slam_outputs} crashed @CI</Tag>} {new_ci_batch.pending_slam_outputs>0 && <Tag>{new_ci_batch.pending_slam_outputs} pending @CI</Tag>} <Tag intent={Intent.WARNING}>New commit</Tag></div>
      ...
      <div style={{flex:'1 1 auto'}}>
          ...
          <div style={{marginTop: '10px', textAlign: 'right'}}><DoneAtTag commit={ref_commit} /> <Tag intent={Intent.SUCCESS}>{ref_ci_batch.valid_slam_outputs} outputs @CI</Tag> {ref_ci_batch.failed_slam_outputs>0 && <Tag intent={Intent.DANGER}>{ref_ci_batch.failed_slam_outputs} crashed @CI</Tag>} {ref_ci_batch.pending_slam_outputs>0 && <Tag>{ref_ci_batch.pending_slam_outputs} pending @CI</Tag>} <Tag intent={Intent.PRIMARY}>Reference</Tag></div>
