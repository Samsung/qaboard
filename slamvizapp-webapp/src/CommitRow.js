...
class CommitRow extends React.Component {
  render() {
    ...
    const commit_url = (project === 'dvs/psp_swip' || project === 'tof/swip_tof') ? `http://gitlab-srv/${project}/commit/${commit.id}` : '#sorry-not-yet-available';
    return (
        ...
        <CommitDetails>
          <CommitContent style={{maxWidth: '600px'}}>
            ...
            <div>
              ...
              <Tooltip>
                <CopyToClipboard text={commit.id} onCopy={() => {toaster.show({ message: "Copied to clipboard!", intent: Intent.PRIMARY})}}>
