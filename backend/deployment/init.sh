...
cd /qaboard/backend && sudo -E /opt/anaconda3/bin/uwsgi --ini /qaboard/backend/deployment/backend.ini &

export QABOARD_DB_ECHO=True
cd /qaboard/backend && FLASK_APP=backend FLASK_DEBUG=1 flask run --host 0.0.0.0 --with-threads --port 5002 &
