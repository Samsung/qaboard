...
qa batch first-batch second-batch
```

## Setting a custom database
In QA-Board relative input paths are considered relative to a "database" folder (defaults to `/` unless changed in _qatools.yaml_).

You can set a default database for all the batches in define in each YAML file:
...
You can also set top-level default `configs` in your YAML files.
...
## Dynamic groups of batches
You can use [wildcards](https://docs.python.org/3/library/fnmatch.html):
...
```bash
qa batch 'batch-*'
#=> runs both batch-1 and batch-2
```

Aliases also understand wildcards:
...
no-hdr:
  <<: *HDR-disabled
  <<: *inputs_hdr
# qa --label hdr    batch hdr
# qa --label no-hdr batch no-hdr
